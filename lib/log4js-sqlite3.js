...
Appender.prototype._getInsertSql = function(){
    ...
    let sql = `INSERT INTO  ${name} (${colNames}) VALUES(${placeholders});`;
    return sql;
}
...
Appender.prototype._writeToLog = async function(logData){
    ...
    const insertSql = this._getInsertSql();

    const formattedData = util.format(...logData.data);
    const sqlArgs = [logData.startTime.getTime(), formattedData, logData.level.levelStr, logData.categoryName];
    if(this.config.additionalFields != null){
        this.config.additionalFields.forEach(x => sqlArgs.push(x.value));
    }
    await this._runCommand(insertSql, sqlArgs);
}
