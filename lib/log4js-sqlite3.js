...
Appender.prototype._runCommand = async function(sqlCommand, parameters) {
    ...
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            db.run(sqlCommand, parameters, function (err) {
                if (err) {
                    reject(err);
                    return;
                };
...
Appender.prototype._ensureSqLiteDbExists = async function(){
    ...
    if(isNewDb){
        ...
        await self._runCommand(`CREATE INDEX "timeIndex" on "${this.table}"("time");`);
    }
...
Appender.prototype._writeToLog = async function(logData){
    ...
    const lastId = await this._runCommand(this.insertSql, sqlArgs);
    if(this.maxRecordCount != null && lastId > this.maxRecordCount){
        const oldestIdToKeep = (lastId - this.maxRecordCount)+1;
        const cleanupSql = `DELETE FROM ${this.table} WHERE ROWID < (${oldestIdToKeep});`;
        debug(cleanupSql);
        await this._runCommand(cleanupSql);
    }
};
